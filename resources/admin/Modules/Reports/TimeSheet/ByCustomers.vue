<template>
    <el-table v-if="isLoaded" :data="customers" style="width: 100%">
        ...
        <el-table-column :min-width="100" v-for="date in dateLabels" :key="date" :prop="date" :label="date" min-width="100">
            ...
            <template #default="{ row }">
                <CustomerDataSheetPop :customer_id="row.id" :date="date" :timeSheets="timeSheets" />
            </template>
        </el-table-column>
...
export default {
    name: 'ByCustomers',
    components: { CustomerDataSheetPop },
    props: {
        mailbox_id: {
            type: String,
            required: true
    ...
    setup(props) {
        ...
        const fetchReportsByAgents = async () => {
            ...
            try {
                const response = await get('reports/timesheet/by-customers', {
