    ...
    <div class="fs_tickets_view">
        <div class="inner_sidebar">
            <ul>
                ...
                <li>
                    <router-link :class="{router_not_exactly_matched: isUnassigned }" :to="{ name: 'tickets', query: { agent_id: 'unassigned' } }"><el-icon> <View/> </el-icon>{{translate('Unassigned')}}</router-link>
                </li>
                <li v-if="has_pro">
                    <router-link :class="{router_not_exactly_matched: isMentioned }" :to="{ name: 'tickets', query: {  watcher: 'watcher', agent_id: appVars.me.id } }">
