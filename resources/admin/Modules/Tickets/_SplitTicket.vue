<template>
    <el-form :data="ticket_data" label-position="top">
        <el-form-item label="Ticket Title">
            <el-input v-model="ticket_data.title" placeholder="Ticket Title" />
        </el-form-item>
        <el-form-item label="Ticket Content">
            <wp-editor v-model="ticket_data.content" />
        </el-form-item>

        <el-row :gutter="30">
            <el-col :md="12" :xs="24">
                <el-form-item label="Ticket Priority">
                    <el-select v-model="ticket_data.client_priority" placeholder="Ticket Priority">
                        <el-option v-for="(priorityValue, priorityKey) in priorities" :key="priorityKey" :value="priorityKey" :label="priorityValue"></el-option>
                    </el-select>
                </el-form-item>
            </el-col>
            <el-col :md="12" :xs="24">
                <el-form-item label="Ticket Product" v-if="products.length">
                    <el-select v-model="ticket_data.product_id" placeholder="Ticket Product">
                        <el-option v-for="product in products" :key="product.id" :value="product.id" :label="product.title"></el-option>
                    </el-select>
                </el-form-item>
            </el-col>
            <el-col :md="12" :xs="24">
                <el-form-item label="Ticket Mailbox" v-if="mailboxes.length">
                    <el-select v-model="ticket_data.mailbox_id" placeholder="Ticket Mailbox">
