    ...
    <Teleport to="body">
        <modal :show="show" title="Import Tickets From HelpScout in Fluent Support" @close="$emit('close')">
            ...
            <span class="dialog-footer">
                <el-checkbox v-if="Object.keys(previously_imported).length > 0 && settings.mailbox_id == previously_imported.mailbox_id" v-model="start_from_previous_migration" :label="$t('resume_helpscout_migration_messsage') + ' (' + previously_imported.completed + '% ' + $t('completed') + ')'" size="large" /><br>
